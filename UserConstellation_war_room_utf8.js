...
/**
 * UserConstellation ΓÇö Real-Time Intent Star Map
 *
 * SVG-based "Star Map" showing live users as glowing dots.
 * Size = time on site, Color = intent score (WhiteΓåÆYellowΓåÆOrangeΓåÆRed).
...
// Intent score ΓåÆ color interpolation
...
// Time on site ΓåÆ dot radius (clamped 4-22)
function timeToRadius(seconds) {
  return Math.min(22, Math.max(4, 4 + seconds / 15));
}

// Deterministic pulse delay class from user_id (1ΓÇô7)
...
export default function UserConstellation() {
  ...
  const usersRef = useRef(new Map()); // userId ΓåÆ user data
          ...
          >
            ...
            {visibleUsers.map((u) => {
              ...
              return (
                ...
                >
                  {/* Halo glow for high-intent ΓÇö cardiac pulse */}
                  {glowR > 0 && (
                    <circle
                      cx={0}
                      cy={0}
                  ...
                  {/* Main dot ΓÇö subtle breathe animation */}
                  <circle
                    cx={0}
                    cy={0}
                    r={r}
