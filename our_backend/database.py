...
async def sync_db_session(session_id: str, telemetry_data: dict, intent_score: int) -> int:
    ...
    try:
        ...
        if res_u.data:
            ...
            if res_u2.data:
                ...
                if res_u3.data:
                    ...
                    print(f"👤 Created new user in database for session {session_id}: ID {user_id}")
        ...
        if res_s2.data:
            ...
            session_to_db[session_id] = session_db_id
            print(f"💾 Created session record in database: ID {session_db_id}")
            return session_db_id
...
async def save_decision_telemetry(
    ...
    try:
        ...
        try:
            ...
            print(f"🧠 Logged behavior analysis to explainability_logs for User {user_id}")
        except Exception as e:
        ...
        if nudge_package and (intervention.get("show_popup") or intervention.get("send_email") or intervention.get("send_whatsapp")):
            try:
                ...
                print(f"✉️ Saved nudge in nudges table for User {user_id}")
            except Exception as e:
        ...
        try:
            ...
            print(f"💰 Updated revenue metrics for User {user_id}")
        except Exception as e:
        ...
        try:
            ...
            print(f"📈 Updated rule performance statistics for Rule {rule_id}")
        except Exception as e:
...
async def save_manual_intervention(session_id: str, offer_type: str, custom_message: str):
    ...
    try:
        ...
        if user_id:
            ...
            print(f"🛠️ Logged manual intervention for User {user_id}")
            ...
            supabase.table("nudges").insert(nudge_payload).execute()
            print(f"✉️ Saved manual nudge record in database for User {user_id}")
    except Exception as e:
