...
# In-memory cache: url → classified stage (avoids re-calling Gemini for same URL)
_stage_cache: dict = {}


async def classify_page_structure(dom_summary: dict) -> dict:
    """
    Analyzes a DOM snapshot and returns its Universal Stage.
    Caches results by page URL to avoid redundant LLM calls.
    """
    # Cache check — Risk 3 fix (latency)
    cache_key = dom_summary.get("page_url", "")
    if cache_key and cache_key in _stage_cache:
        cached = _stage_cache[cache_key]
        logger.info(f"[SEMANTIC MAPPER] Cache hit for {cache_key} → {cached['stage']}")
        return cached
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[SEMANTIC MAPPER] Classified as {result.get('stage')} (conf: {result.get('confidence')})")
        ...
        logger.error(f"[SEMANTIC MAPPER] Error: {e}")
        return {
            "stage": "Exploration",
            "confidence": 0.1,
